...
logger.info(
    f"Initialized WacoStreetsAnalyzer with {len(waco_analyzer.streets_gdf)} streets")
...
def create_app():
    ...
    async def get_untraveled_streets():
        ...
        untraveled_streets = geojson_handler.get_untraveled_streets(
            waco_boundary)
        ...
        async with app.historical_data_lock:
            try:
                ...
                if params.filter_waco and params.waco_boundary != "none":
                    waco_limits = geojson_handler.load_waco_boundary(
    ...
    async def update_historical_data():
        async with app.processing_lock:
            ...
            try:
                ...
                # Call the method from geojson_handler
    ...
    async def get_progress_geojson():
        try:
            waco_boundary = request.args.get("wacoBoundary", "city_limits")
            progress_geojson = geojson_handler.get_progress_geojson(
                waco_boundary)
    ...
    async def get_waco_streets():
        try:
            ...
            streets_geojson = geojson_handler.get_waco_streets(
    ...
    def create_empty_historical_data_file():
        ...
        if not os.path.exists(filename):
            logger.info(
                f"No historical data files found. Creating an empty one: {filename}")
            empty_geojson = {
                "type": "FeatureCollection",
